...
from extract_text_from_pdf import return_trading_info
...
# Ticker for OANDA needs to be like "GBP_AUD", for TradingView "GBPAUD"
ticker_oanda = "GBP_AUD"
ta_symbol = "GBPAUD" # TradingView symbol for EUR/USD
screener = "forex"
exchange = "FX_IDC" # This is often used for forex on TradingView
interval_tv = "5m" # TradingView interval
interval_oanda_granularity = "M5" # OANDA granularity for 5-minute candles
# For historical data, OANDA uses 'count' or 'from/to' timestamps.
OANDA_HISTORICAL_CANDLE_COUNT = 5000 # Roughly 1 month of 5-min candles
...
def get_candlestick_patterns(open_prices, high_prices, low_prices, close_prices):
    ...
    # Use .item() to extract the scalar value from the Series returned by .iloc[-1]
...
def fetch_oanda_data(instrument, granularity, count=None, _from=None, _to=None):
    """
    Fetches historical candlestick data from OANDA.
    Provide either `count` or both `_from` and `_to` timestamps.
    """
    params = {"granularity": granularity}
    if count:
        params["count"] = count
    if _from and _to:
        params["from"] = _from.isoformat("T") + "Z" # ISO 8601 format with Z for UTC
        params["to"] = _to.isoformat("T") + "Z"

    r = instruments.InstrumentsCandles(instrument=instrument, params=params)
    client_oanda.request(r) # Use client_oanda here
    ...
    if r.status_code == 200:
        ...
        print(f"Error fetching OANDA data: {r.status_code} - {r.response}")
        return pd.DataFrame() # Return empty DataFrame on error


def get_historical_data(ticker_oanda, interval_oanda_granularity, candle_count):
    """
    Fetches historical data using OANDA API and calculates historical ADX state and candlestick patterns.
    Updates global variables historical_adx_state and historical_candlestick_patterns.
    Returns the full historical ADX Series, historical candlestick pattern counts, ADX state string,
    and price series (open, high, low, close, volume).
    """
    global historical_adx_state, historical_candlestick_patterns, data

    print(f"Fetching historical data for {ticker_oanda} with granularity {interval_oanda_granularity} (count={candle_count}) from OANDA...")
    data = fetch_oanda_data(ticker_oanda, interval_oanda_granularity, count=candle_count)

    if data.empty or len(data) < 20: # Ensure enough data for indicators (e.g., ADX needs 14-20 periods)
        print("Not enough historical data retrieved from OANDA for indicator calculations.")
    ...
    # Ensure column names are correct and data types are float64 for TA-Lib and `ta`
    open_prices_series = pd.Series(data["open"].values.astype(np.float64))
    close_prices_series = pd.Series(data["close"].values.astype(np.float64))
    high_prices_series = pd.Series(data["high"].values.astype(np.float64))
    low_prices_series = pd.Series(data["low"].values.astype(np.float64))
    volume_series = pd.Series(data["volume"].values.astype(np.float64)) # Volume as float64
...
def get_current_data(ticker_oanda, interval_oanda_granularity, exchange, screener, ta_symbol):
    """
    ...
    print(f"Fetching current data for {ticker_oanda} with granularity {interval_oanda_granularity} (count=100) from OANDA...")
    data = fetch_oanda_data(ticker_oanda, interval_oanda_granularity, count=100)

    # Check if data is empty or has less than 2 rows for pivot point calculation
    if data.empty or len(data) < 2:
        print(f"Not enough data retrieved for {ticker_oanda} with interval {interval_oanda_granularity}. Need at least 2 candles for pivot points and indicators.")
    ...
    high_prices_series = pd.Series(data["high"].values.astype(np.float64))
    low_prices_series = pd.Series(data["low"].values.astype(np.float64))
    volume_series = pd.Series(data["volume"].values.astype(np.float64))
    ...
    latest_hammer = ta_lib.CDLHAMMER(open_prices_series, high_prices_series, low_prices_series, close_prices_series).iloc[-1].item()
    latest_engulfing = ta_lib.CDLENGULFING(open_prices_series, high_prices_series, low_prices_series, close_prices_series).iloc[-1].item()
    latest_doji = ta_lib.CDLDOJI(open_prices_series, high_prices_series, low_prices_series, close_prices_series).iloc[-1].item()
    ...
    # Display TA Library and OANDA data
    print("\n---------TA Library and OANDA Data---------\n")
    print(f"TA-Lib Data for {ticker_oanda} ({interval_oanda_granularity}):")
    ...
    try:
        handler = TA_Handler(
            ...
            screener=screener,
            exchange=exchange,
            interval=interval_tv # Use the TradingView specific interval here
        )
        ...
        print("\n---------TradingView Data---------\n")
        print(f"TradingView Data for {ta_symbol} ({exchange}, {interval_tv}):")
    ...
    hist_adx_series, hist_candlesticks, hist_adx_state_str, \
        _, _, _, _, _ = \
        get_historical_data(ticker_oanda, interval_oanda_granularity, OANDA_HISTORICAL_CANDLE_COUNT)
...
def send_to_gemini(prompt_text):
    ...
    try:
        ...
        trading_info = return_trading_info()
        time_now = datetime.datetime.now(pytz.timezone("Australia/Sydney")).strftime("%Y-%m-%d %H:%M:%S")
        response = gemini_model.generate_content(f"I am living in Melbourne so the time is the +10UTC Sydney time and my time is {time_now}" + prompt_text + "I have attached a few PDF's with trading strategies, use that to help you to make your decision and make sure to add the reasoning behind it and what the strategies tell you, do all of the PDf's, " + trading_info, generation_config=generation_config)
...
def main():
    ...
    ticker_oanda = "GBP_AUD"
    interval_oanda_granularity = "M5"
    exchange = "FX_IDC"  # Example for TradingView
    screener = "forex"   # Example for TradingView
    ta_symbol = "GBPAUD" # Example for TradingView symbol
    
    while True:

        # Retrieve current data and initial market regime analysis
        current_data_info = get_current_data(ticker_oanda, interval_oanda_granularity, exchange, screener, ta_symbol)
        ...
        latest_atr_ta_lib = _safe_float(get_latest_indicator_value(current_data_info['live_indicators']['atr']))

        # Populate a dictionary with all data points for Gemini
        interval_tv = "15m" # Ensure this matches your TradingView interval for the prompt context
    ...
    Here is also the past data of the candles, indicators and candlestick patterns, this is the last months candlesticks
    Historical ADX: {historical_adx_state}
    Historical Candlestick patterns: {historical_candlestick_patterns}
    Last Month's Candles: {data}
    ...
    "--- Calculated Pivot Points ---": {{
        ...
        if gemini_response_text:
            ...
            if parsed_gemini_data:
                ...
                if decision == "BUY":
                    ...
                    estimated_take_profit = latest_close_price + (2.5 * latest_atr_ta_lib)
                    send_email("daasrmalik@gmail.com", "GBPAUD - Trading Decision - BUY", f"Gemini's decision is to BUY {ta_symbol} at {latest_close_price:.5f} with ATR {latest_atr_ta_lib:.5f}. and the Take Profit is {estimated_take_profit:.5f} and the Stop Loss is {estimated_stop_loss:.5f}")
                elif decision == "SELL":
                    estimated_entry_price = latest_close_price
                    estimated_stop_loss = latest_close_price + (1.5 * latest_atr_ta_lib)
                    estimated_take_profit = latest_close_price - (2.5 * latest_atr_ta_lib)
                    send_email("daasrmalik@gmail.com", "GBPAUD - Trading Decision - SELL", f"Gemini's decision is to SELL {ta_symbol} at {latest_close_price:.5f} with ATR {latest_atr_ta_lib:.5f}. and the Take Profit is {estimated_take_profit:.5f} and the Stop Loss is {estimated_stop_loss:.5f}")
                else: # HOLD or other
                ...
                decision_section['Estimated Entry Price'] = f"{estimated_entry_price:.5f}" if estimated_entry_price else 'N/A'
        ...
        # wait for 5 mins to avoid rate limiting and to allow new data to accumulate
        print("Waiting for 5 minutes before the next analysis...")
        time.sleep(300)
