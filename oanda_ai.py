...
# Ticker for OANDA needs to be like "GBP_JPY", for TradingView "GBPJPY"
ticker_oanda = "GBP_JPY"
ta_symbol = "GBPJPY" # TradingView symbol for EUR/USD
screener = "forex"
...
def main():
    ...
    ta_symbol = "GBPJPY" # Example for TradingView symbol
        ...
        if gemini_response_text:
            ...
            if parsed_gemini_data:
                ...
                if decision == "BUY":
                    ...
                    send_email("daasrmalik@gmail.com", "Trading Decision - BUY", f"Gemini's decision is to BUY {ta_symbol} at {latest_close_price:.5f} with ATR {latest_atr_ta_lib:.5f}. and the Take Profit is {estimated_take_profit:.5f} and the Stop Loss is {estimated_stop_loss:.5f}")
                elif decision == "SELL":
                    estimated_entry_price = latest_close_price
                    estimated_stop_loss = latest_close_price + (1.5 * latest_atr_ta_lib)
                    estimated_take_profit = latest_close_price - (2.5 * latest_atr_ta_lib)
                    send_email("daasrmalik@gmail.com", "Trading Decision - SELL", f"Gemini's decision is to SELL {ta_symbol} at {latest_close_price:.5f} with ATR {latest_atr_ta_lib:.5f}. and the Take Profit is {estimated_take_profit:.5f} and the Stop Loss is {estimated_stop_loss:.5f}")
                else: # HOLD or other
